...
| RF03 | O sistema deve permitir cadastrar totens. |
| RF04 | O sistema deve permitir cadastrar produtos. |
| RF05 | O sistema deve permitir registrar pedidos. |
| RF06 | O sistema deve identificar o canal de realização do pedido: TOTEM, BALCÃO ou APLICATIVO. |
| RF07 | O sistema deve permitir registrar os produtos e quantidades de cada pedido. |
| RF08 | O sistema deve registrar o preço praticado no momento da venda. |
| RF09 | O sistema deve permitir registrar um ou mais pagamentos para um pedido. |
| RF10 | O sistema deve permitir registrar pagamentos divididos entre diferentes formas de pagamento. |
| RF11 | O sistema deve permitir consultar a quantidade disponível de cada produto. |
| RF12 | O sistema deve permitir registrar entradas de estoque. |
| RF13 | O sistema deve permitir cadastrar fornecedores. |
| RF14 | O sistema deve registrar os produtos e quantidades recebidos em cada entrada de estoque. |
| RF15 | O sistema deve permitir consultar pedidos e pagamentos. |
| RF16 | O sistema deve permitir consolidar informações de vendas para apoiar o fechamento mensal. |

## 3.2 Requisitos Não Funcionais

| ID | Requisito |
|---|---|
| RNF01 | **Usabilidade:** as informações devem ser apresentadas de forma clara e organizada. |
| RNF02 | **Integridade:** os relacionamentos entre pedidos, produtos, pagamentos e estoque devem permanecer consistentes. |
| RNF03 | **Segurança:** informações administrativas e financeiras devem possuir controle de acesso adequado. |
| RNF04 | **Desempenho:** consultas de pedidos, estoque e vendas devem apresentar resposta adequada à operação. |
| RNF05 | **Disponibilidade:** as informações necessárias à operação devem estar disponíveis quando forem necessárias. |
| RNF06 | **Escalabilidade:** o modelo deve permitir o crescimento da quantidade de produtos, pedidos, funcionários e fornecedores. |
...
| ID | Regra |
|---|---|
| RN01 | Todo pedido deve possuir um identificador único. |
| RN02 | Todo pedido deve possuir pelo menos um item. |
| RN03 | A quantidade de um item de pedido deve ser maior que zero. |
| RN04 | Cada item de pedido deve estar associado a um produto cadastrado. |
| RN05 | O preço unitário registrado no item deve representar o valor praticado no momento da venda. |
| RN06 | Um pedido pode possuir mais de um pagamento para permitir divisão da conta. |
| RN07 | Cada pagamento deve registrar o valor correspondente àquela forma de pagamento. |
| RN08 | A quantidade disponível no estoque não pode ser negativa. |
| RN09 | Toda entrada de estoque deve registrar fornecedor, data e produtos recebidos. |
| RN10 | A quantidade recebida de um produto deve ser maior que zero. |
| RN11 | Cada pedido deve possuir um canal: TOTEM, BALCÃO ou APLICATIVO. |
| RN12 | Pedido realizado pelo totem deve possuir referência ao totem utilizado. |
| RN13 | Pedido realizado no balcão deve possuir referência ao funcionário responsável pelo registro. |
| RN14 | Pedido realizado pelo aplicativo não precisa estar associado a funcionário ou totem da unidade. |
| RN15 | O cliente pode não possuir cadastro completo no momento do pedido. |
| RN16 | CPF/CNPJ do cliente é opcional. |

## 4.2 Restrições Organizacionais

| ID | Restrição | Impacto no modelo |
|---|---|---|
| RO01 | Atualmente existe um fornecedor principal, mas outros fornecedores podem ser utilizados quando houver necessidade e autorização. | O modelo permite cadastrar múltiplos fornecedores. |
| RO02 | O estoque é controlado individualmente por produto. | Cada produto possui um registro de estoque. |
| RO03 | A unidade registra quantidade e data das entradas de estoque. | Foram criadas ENTRADA_ESTOQUE e ITEM_ENTRADA. |
| RO04 | Não foi identificado controle individual de validade no processo levantado. | O atributo de validade não foi incluído no modelo. |
| RO05 | O aplicativo utiliza sua própria plataforma e entrada de pedidos. | O pedido é identificado pelo canal APLICATIVO, sem associação obrigatória a totem ou funcionário. |
| RO06 | O sistema existente possui relação com o ambiente TOTVS. | O modelo proposto representa os dados levantados e não assume a substituição do sistema existente. |
